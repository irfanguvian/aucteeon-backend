function getProductHotTodayHandlerComposer(diHash) {
  const {
    model,
    moment,
    Sequlieze,
  ...
  async function getProductHotTodayHandler(req, res) {
    try {
      ...
      const ProductsList = await Products.findAndCountAll({
        ...
        order: [[Sequlieze.literal("(SELECT COUNT(*) FROM product_bid WHERE product_bid.product_id = products.id)"), "DESC"]],
        limit,
        offset,
      });
  ...
  return getProductHotTodayHandler;
}

module.exports = getProductHotTodayHandlerComposer;
