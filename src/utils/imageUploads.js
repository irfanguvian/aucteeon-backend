...
async function imageUpload(file, index = 0, imgType = "img") {
  const remotePath = `${process.env.IMAGE_SERVER_ADDRESS}/assets/img/`;

  const _uploadFilesDirectory = `${process.env.STORAGE_PUBLIC}/image/`;
  if (!fs.existsSync(_uploadFilesDirectory)) {
    await fs.mkdirSync(_uploadFilesDirectory);
  }
  ...
  try {
    ...
    await fs.writeFileSync(filePath, imageBuffer, "utf8");
    return urlPath;
  } catch (e) {
